...
server.setRequestHandler(InitializeRequestSchema, async (request) => {
  // Try to get API key from initialization options or environment
  // The MCP SDK may pass initializationOptions in different ways depending on the client
  ...
  // Initialize API client if API key is provided, otherwise defer initialization
  // This allows the inspector to connect and list tools without requiring an API key upfront
  if (apiKey) {
    try {
      apiClient = new PumbleApiClient(apiKey);
    } catch (error) {
      console.error("Failed to initialize Pumble API client:", error);
      // Continue initialization even if API key is invalid
      // Error will be shown when tools are called
    }
  }
